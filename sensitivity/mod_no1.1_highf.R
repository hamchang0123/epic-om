...
## Remove c1.1
mod_no_c1.1_highf <- function(time, state, parameters) {
...
#--MODEL OUTPUT
output_no_c1.1_highf <- as.data.frame(ode(y = initial, 
                                times = times, 
                                func = mod_no_c1.1_highf,
                                parms = parameters_highf))


#--Add 'reported' daily incidence estimated by the model
output2_no_c1.1_highf <- output_no_c1.1_highf %>%
  mutate(
...
p1.1_highf <- ggplot() + 
  geom_line(data = output2_no_c1.1_highf, aes(x=time, y=Dt_per_m, colour='without'))+
  ...
  scale_x_continuous(breaks=c(0, 64, 165))+
