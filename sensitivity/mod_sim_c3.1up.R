...
mod_sim_c3.1up <- function(time, state, parameters) {
  with(as.list(c(state, parameters)),{
    ...
    if (time == 19) {
      ...
      c_yy =  c_yy * intv_c3.1_y_up 
      c_ya = c_ya * intv_c3.1_a_up
      c_ay = c_ay * intv_c3.1_y_up
      c_aa = c_aa_c2.2 * intv_c3.1_a_up}
...
#--MODEL OUTPUT
output_sim_c3.1up <- as.data.frame(ode(y = initial, 
                                times = times, 
                                func = mod_sim_c3.1up,
                                parms = parameters))
...
output2_sim_c3.1up <- output_sim_c3.1up %>%
  mutate(
