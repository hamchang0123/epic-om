...
mod_no_c4.4_lowf <- function(time, state, parameters) {
...
#--MODEL OUTPUT
output_no_c4.4_lowf <- as.data.frame(ode(y = initial, 
                                    times = times, 
                                    func = mod_no_c4.4_lowf,
                                    parms = parameters_lowf))


#--Add 'reported' daily incidence estimated by the model
output2_no_c4.4_lowf <- output_no_c4.4_lowf %>%
  mutate(
    D_y = Ic_y * parameters_lowf['frac_rep'],
    Da_per_m = Ic_a * parameters_lowf['frac_rep'],
    D_t = D_y + Da_per_m,
    Dy_per_m = D_y / parameters_lowf['popn_y'] *1E6,
    Da_per_m = Da_per_m / parameters_lowf['popn_a'] *1E6,
...
p4.4_lowf <- ggplot() + 
  geom_line(data = output2_no_c4.4_lowf, aes(x=time, y=Dt_per_m, colour = 'without'), linetype=1)+
  geom_line(data=output2_sim_lowf, aes(x=time, y=Dt_per_m,colour = "baseline"),linetype=2)+
  labs(x = 'Time (day)', y = 'Daily confirmed incident cases\n(per million people)\n')+
  scale_x_continuous(breaks=c(0, 53, 165))+
  ggtitle('(C)')+
