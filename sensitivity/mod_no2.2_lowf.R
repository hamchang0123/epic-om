...
## Remove c2.2
mod_no_c2.2_lowf <- function(time, state, parameters) {
...
#--MODEL OUTPUT
output_no_c2.2_lowf <- as.data.frame(ode(y = initial, 
                                times = times, 
                                func = mod_no_c2.2_lowf,
                                parms = parameters_lowf))


#--Add 'reported' daily incidence estimated by the model
output2_no_c2.2_lowf <- output_no_c2.2_lowf %>%
  mutate(
...
p2.2_lowf <- ggplot() + 
  geom_line(data = output2_no_c2.2_lowf, aes(x=time, y=Dt_per_m, colour='without'))+
  ...
  scale_x_continuous(breaks=c(0, 68, 165))+
