...
## Remove c1.1
mod_no_c1.1_lowf <- function(time, state, parameters) {
...
#--MODEL OUTPUT
output_no_c1.1_lowf <- as.data.frame(ode(y = initial, 
                                times = times, 
                                func = mod_no_c1.1_lowf,
                                parms = parameters_lowf))


#--Add 'reported' daily incidence estimated by the model
output2_no_c1.1_lowf <- output_no_c1.1_lowf %>%
  mutate(
...
p1.1_lowf <- ggplot() + 
  geom_line(data = output2_no_c1.1_lowf, aes(x=time, y=Dt_per_m, colour='without'))+
  ...
  scale_x_continuous(breaks=c(0, 67, 165))+
