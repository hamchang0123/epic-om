...
mod_sim_c3.1down1 <- function(time, state, parameters) {
  with(as.list(c(state, parameters)),{
    ...
    if (time == 19) {
      ...
      c_yy =  c_yy * intv_c3.1_y_down1 
      c_ya = c_ya * intv_c3.1_a_down1
      c_ay = c_ay * intv_c3.1_y_down1
      c_aa = c_aa_c2.2 * intv_c3.1_a_down1}
...
#--MODEL OUTPUT
output_sim_c3.1down1 <- as.data.frame(ode(y = initial, 
                                times = times, 
                                func = mod_sim_c3.1down1,
                                parms = parameters))


#--Add 'reported' daily incidence estimated by the model
output2_sim_c3.1down1 <- output_sim_c3.1down1 %>%
  mutate(
