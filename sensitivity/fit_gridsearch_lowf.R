#####################
pacman::p_load(rio, tidyverse, ggpubr, deSolve)
...
#######################
k=seq(0.07, 0.09, 0.001)


frac_rep=seq(0.10, 0.30, 0.01)
...
for(i in k){
  ...
  for(j in frac_rep){
    ...
    parameters_lowf['k'] <- i
    parameters_lowf['frac_rep'] <- j
    
    output_f_lowf <- as.data.frame(ode(y = initial, 
                                  times = times, 
                                  func = mod_sim_lowf,
                                  parms = parameters_lowf))
    
    output2_f_lowf <- output_f_lowf %>%
      mutate(
        D_y = Ic_y * parameters_lowf['frac_rep'],
        D_a = Ic_a * parameters_lowf['frac_rep'], 
        D_t = (Ic_y + Ic_a) * parameters_lowf['frac_rep']
      )
    
    L <- sum(dpois(obs_t$daily, lambda = output2_f_lowf$D_t, log=T))
...
ggplot(data = df_sto_est, aes(x = k, y= frac_rep)) +
  ...
  scale_y_continuous(breaks = seq(0.09, 0.13, 0.01))
...
ggplot(data = df_sto_est, aes(x = k, y= frac_rep)) +
  geom_tile(aes(fill=L))+
  scale_fill_viridis_c()+
  scale_x_continuous(breaks = seq(0.075, 0.085, 0.001))+
  scale_y_continuous(breaks = seq(0.1075, 0.1085, 0.0001))+
  theme_classic2()+
  labs(x = '\nk', y = 'frac_rep\n')#+
