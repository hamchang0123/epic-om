...
##### MODEL
mod_sim_c4.3_long <- function(time, state, parameters) {
  with(as.list(c(state, parameters)),{
    ...
    if (time == 19) {
      c_yy = c_yy * intv_c1.1 * intv_c4.3_y 
      c_ya = c_ya * intv_c4.3_a
      c_ay = c_ay * intv_c4.3_y
      c_aa = c_aa_c2.2_max * intv_c4.3_a}
    
    else if (time < 22){
      c_yy =  c_yy * intv_c1.1 * intv_c4.3_y 
      c_ya = c_ya * intv_c4.3_a
      c_ay = c_ay * intv_c4.3_y
      c_aa = c_aa_c2.2 * intv_c4.3_a}
    ...
    #44,72
      ...
      c_yy =  c_yy * intv_c3.1_y * intv_c4.3_y 
      c_ya = c_ya * intv_c3.1_a * intv_c4.3_a
      c_ay = c_ay * intv_c3.1_y * intv_c4.3_y
      c_aa = c_aa_c2.2 * intv_c3.1_a * intv_c4.3_a
      }
...
#--MODEL OUTPUT
output_sim_c4.3_long <- as.data.frame(ode(y = initial, 
                                     times = times, 
                                     func = mod_sim_c4.3_long,
                                     parms = parameters))


#--Add 'reported' daily incidence estimated by the model
output2_sim_c4.3_long  <- output_sim_c4.3_long %>%
  mutate(
