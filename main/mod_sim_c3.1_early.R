...
mod_sim_early3.1 <- function(time, state, parameters) {
  with(as.list(c(state, parameters)),{
    ...
    if (time == 19) {
      c_yy = c_yy * intv_c1.1 * intv_c4.4_y * intv_c3.1_y 
      c_ya = c_ya * intv_c4.4_a * intv_c3.1_a
      c_ay = c_ay * intv_c4.4_y * intv_c3.1_y 
      c_aa = c_aa_c2.2_max * intv_c4.4_a * intv_c3.1_a}
    
    else if (time < 22){
      c_yy =  c_yy * intv_c1.1 * intv_c4.4_y * intv_c3.1_y 
      c_ya = c_ya * intv_c4.4_a * intv_c3.1_a
      c_ay = c_ay * intv_c4.4_y * intv_c3.1_y 
      c_aa = c_aa_c2.2 * intv_c4.4_a * intv_c3.1_a}
    ...
    #In fact, OxCGRT team assumed intv_c3.2 was in place when intv_c4.4. 
      ...
      c_yy =  c_yy * intv_c4.4_y * intv_c3.1_y 
      c_ya = c_ya * intv_c4.4_a * intv_c3.1_a
      c_ay = c_ay * intv_c4.4_y * intv_c3.1_y 
      c_aa = c_aa_c2.2 * intv_c4.4_a * intv_c3.1_a
    }
...
#--MODEL OUTPUT
output_sim_early3.1 <- as.data.frame(ode(y = initial, 
                                times = times, 
                                func = mod_sim_early3.1,
                                parms = parameters))


#--Add 'reported' daily incidence estimated by the model
output2_sim_early3.1 <- output_sim_early3.1 %>%
  mutate(
    ...
    Dy_per_m_t = D_y / parameters['popn'] *1E6,
    Da_per_m_t = D_a / parameters['popn'] *1E6
  )
