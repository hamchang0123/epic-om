...
mod_sim_c1.1_long <- function(time, state, parameters) {
  with(as.list(c(state, parameters)),{
    ...
    #In fact, OxCGRT team assumed intv_c3.2 was in place when intv_c4.4. 
      ...
      c_yy =  c_yy * intv_c4.4_y * intv_c1.1
      c_ya = c_ya * intv_c4.4_a
      c_ay = c_ay * intv_c4.4_y
      c_aa = c_aa_c2.2 * intv_c4.4_a
    }
      ...
      c_yy =  c_yy * intv_c3.1_y * intv_c1.1
...
#--MODEL OUTPUT
output_sim_c1.1_long <- as.data.frame(ode(y = initial, 
                                times = times, 
                                func = mod_sim_c1.1_long,
                                parms = parameters))


#--Add 'reported' daily incidence estimated by the model
output2_sim_c1.1_long <- output_sim_c1.1_long  %>%
  mutate(
...
cumsum(output2_sim_c1.1_long$Dt_per_m)[166]
  #67516
max_t_sim_c1.1_long <- output2_sim_c1.1_long %>%
